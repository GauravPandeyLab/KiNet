...
```
Use mouse to select and move nodes, pan and zoom on graph.
Double-click node to re-center the graph on that node.

#### Selected node: `r renderText(input$centerProtein)`
...
## Main Panel
#### Local neighborhood of `r renderText(input$centerProtein)`
### {data-height="800"}
...
output$network <- renderVisNetwork({
 ...
 V(subg)$group = sapply(V(subg)$name,get_group)
 ...
 visNetwork(nodes=data$nodes,edges=data$edges,physics=T) %>%
   ...
   visNodes(id=input$centerProtein,size=50) %>%
