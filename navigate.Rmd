...
```{r, cache=FALSE}
  ...
  selectizeInput('download_choice1',label=NULL,choices=export_choices)
  filename1 <- reactive({switch(input$download_choice1,"Nodes as CSV"='nodes.csv', "Edges as CSV"='edges.csv')})
  (downloadButton('download1',"Download"))
  
  output$download1 <- downloadHandler(
    filename=function() {filename1()},
    content = function(file) {
      if(filename1()=='nodes.csv') {write.csv(g1()$nodes,file,row.names=F)}
      if(filename1()=='edges.csv') {
