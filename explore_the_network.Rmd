...
output$network1 <- renderVisNetwork({
  ...
  visEvents(selectEdge = "function(edges) {Shiny.setInputValue('selectEdge', edges.edges);}")
   })
...
infoEdge <- reactive({
  e <- input$selectEdge
  if (!is.null(e) & length(e)==1) {all_edges %>% filter(id==e)} else {'Nope'}
  })
